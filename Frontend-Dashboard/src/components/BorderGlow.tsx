...
const BorderGlow: React.FC<BorderGlowProps> = ({
  ...
  const [edgeProximity, setEdgeProximity] = useState(alwaysOn ? 1 : 0)
  ...
  const isVisible = alwaysOn || isHovered || sweepActive
  const borderOpacity = alwaysOn
    ? 1
    : isVisible
      ? Math.max(0, (edgeProximity * 100 - colorSensitivity) / (100 - colorSensitivity))
      : 0
  const glowOpacity = alwaysOn
    ? 1
    : isVisible
